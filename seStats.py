...
def loadMissTrans():
    ...
    d = dict()
    fin = open("%s/Translate/MissTranSEsInp.txt" % params.OUTPUT_DIR)
    fin2 = open("%s/Translate/MissTranSEsOut.txt" % params.OUTPUT_DIR)
    while True:
        line = fin.readline()
        if line == "":
            break
...
def loadValidDis(t=0):
    lineIns = open("%s/SEs/seStats.txt" % params.OUTPUT_DIR).readlines()
    d = dict()
    for line in lineIns:
...
def exportCaseSes():
    ...
    cc = 0
    d = loadMissTrans()
    validDis = loadValidDis(SE_T)

    fout = open("%s/SEs/CaseSE.txt" % params.OUTPUT_DIR, "w")
...
def statsSEs():
    ...
    fout = open("%s/SEs/seStats.txt" % params.OUTPUT_DIR, "w")
...
def exportMissTrans():
    lineIns = open("%s/SEs/seStats.txt" % params.OUTPUT_DIR).readlines()
    fout = open("%s/Translate/MissTranSEsInp.txt" % params.OUTPUT_DIR, "w")
    for line in lineIns:
